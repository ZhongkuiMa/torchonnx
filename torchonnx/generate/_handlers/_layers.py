...
from ...analyze import SemanticLayerIR, VariableInfo


def _handle_generic_layer(
    layer: SemanticLayerIR, layer_name_mapping: dict[str, str]
) -> str:
    """Generic handler for layer operations.

    Generates: output = self.layer_name(input)
    ...
    input_names = [
        inp.code_name for inp in layer.inputs if isinstance(inp, VariableInfo)
    ]
...
def _handle_batchnorm2d(
    layer: SemanticLayerIR, layer_name_mapping: dict[str, str]
) -> str:
    """Handler for BatchNorm2d with 3D input support.

    BatchNorm2d expects 4D input (N, C, H, W). For 3D ONNX inputs (N, C, L),
    we need to reshape to (N, C, 1, L) using unsqueeze(2), then squeeze(2) back.
    ...
    input_info = next(
        (inp for inp in layer.inputs if isinstance(inp, VariableInfo)), None
    )
    ...
    if input_shape and len(input_shape) == 2:
        ...
        return f"{output_name} = self.{clean_name}({input_name}.unsqueeze(2).unsqueeze(3)).squeeze(2).squeeze(2)"
    elif input_shape and len(input_shape) == 3:
        # 3D input: add one spatial dimension
        return (
            f"{output_name} = self.{clean_name}({input_name}.unsqueeze(2)).squeeze(2)"
        )
    elif input_shape and len(input_shape) == 4:
