    ...
    if assignment_match:
        ...
        if len(args) > 1:
            ...
            for i, arg in enumerate(args):
                ...
                # Check if this argument line is still too long
                if len(arg_line) > MAX_LINE_LENGTH:
                    # Try to wrap long ternary expressions or other patterns
                    wrapped_arg = _wrap_long_argument(arg, arg_indent)
                    if wrapped_arg:
                        result.extend(wrapped_arg)
                        result[-1] += comma  # Add comma to last line
                    else:
...
def _wrap_long_argument(arg: str, indent: str) -> list[str] | None:
    """Wrap a long argument across multiple lines.

    Handles patterns like:
    - name=(a if condition else b)  -> ternary expression
    - name=long_expression

    :param arg: Argument string to wrap
    :param indent: Base indentation for the argument
    :return: List of wrapped lines, or None if can't wrap
    """
    ...
    if ternary_match:
        ...
        condition = ternary_match.group(3).strip()
        false_expr = ternary_match.group(4).strip()
        return [
            f"{indent}{name}=(",
            f"{continuation_indent}{true_expr}",
            f"{continuation_indent}if {condition}",
            f"{continuation_indent}else {false_expr}",
            f"{indent})",
        ]
    ...
    if ternary_match2:
        ...
        condition = ternary_match2.group(3).strip()
        false_expr = ternary_match2.group(4).strip()
        return [
            f"{indent}{name}=(",
            f"{continuation_indent}{true_expr}",
            f"{continuation_indent}if {condition}",
